...
//const {Vector2, Vector3, Vector4, Color} = THREE;
...
function draw_obj (vp_mat) {
    ...
    config.SPEED_X *= 0.9; // dissipation of x-roll speed
    config.SPEED_Y *= 0.9; // dissipation of y-roll speed    
    g_model_mat.set(x_roll_mat); // cumulative x-roll    
    g_model_mat.multiply(y_roll_mat); // post-multiply cumalative y-roll (thus applied first)
    x_roll_mat.rotate(config.SPEED_X, 1, 0, 0); // update cumulative x-roll
    y_roll_mat.rotate(config.SPEED_Y, 0, 1, 0); // update cumulative y-roll
    ...
    // Draw
    //gl.drawElements(gl.TRIANGLES, g_data.indices.length, gl.UNSIGNED_SHORT, 0);
...
function cg_wheel (e) {
	if (e.deltaY > 0) { // wheel down (zoom out)
	   config.CAMERA_DIST += 5.0;
	   render();
	}
	else { // wheel up (zoom in)
	   config.CAMERA_DIST -= 5.0;
	   if (config.CAMERA_DIST < 5) config.CAMERA_DIST = 5.0; // min distance from origin
	   render();
	}
}
